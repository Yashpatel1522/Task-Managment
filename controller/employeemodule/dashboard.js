...
const getdashboardata = async (request, response) => {
  try {
    ...
    let employeeRecentActivityTeamData = await db.executeQuery(employeeRecentActivityTeamQuery,[id])
    let employeeRecentActivityTaskData = await db.executeQuery(employeeRecentActivityTaskQuery,[id])

    res.employeeRecentActivityData =employeeRecentActivityTeamData.concat(employeeRecentActivityTaskData).sort((a,b) => {
...
const dashboard = async (request, response) => {
  ...
  [taskStatusCounts] = await db.executeQuery(employeeTaskStatusCountsQuery, 4);

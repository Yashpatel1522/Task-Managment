...
const getdashboardata = async (request,response) => {
    try{
        ...
        let employeeUpCommingDeadlineQuery =  `select tasks.id as '#', task_name as Task, task_end_date as DueDate, urgency.type as Type from tasks_assigend_to inner join tasks on tasks_assigend_to.task_id=tasks.id inner join priorities on priorities.id = tasks.prioritiy_id inner join urgency on urgency.id = priorities.urgency_id where emp_id = 1 and task_end_date >= '${currentDate}';`
        ...
        console.log(employeeUpCommingDeadlineData);

...
const managerProfile = async (request,response) => {
    try{
        ...
        let urgencyQ = `select type from urgency where id in (select id from priorities where id in (select prioritiy_id from tasks where id = ? and status = 1));`;
        let urgencyRes = await db.executeQuery(urgencyQ, [request.params.id]); 

        let importanceQ = `select type from importants where id in (select id from priorities where id in (select prioritiy_id from tasks where id = ? and status = 1));`;

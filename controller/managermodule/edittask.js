...
const editTask = async (request, response) => {
  try {
    ...
    let taskRes = await db.executeQuery(tasksUpdate, [
    ...
    let priorityRes = await db.executeQuery(getPriority, [
      request.body.Urgency,
      request.body.importance,
    ]);

    let updatedPriority = `update tasks set prioritiy_id = ? where id = ?;`;
    let updatePriorityRes = await db.executeQuery(updatedPriority, [
      priorityRes[0].id,
      request.body.id,
    ]);

    let deleteEmployee = `delete from tasks_assigend_to where task_id = ?;`;
    let delEmployeeRes = await db.executeQuery(deleteEmployee, [
      request.body.id,
    ]);
    ...
    if (request.body.emp) {
      if (Array.isArray(request.body.emp)) {
        request.body.emp.forEach(async function (element) {
          ...
          let insEmployeeRes = await db.executeQuery(insertEmployee, [
            request.body.id,
            element,
          ]);
        });

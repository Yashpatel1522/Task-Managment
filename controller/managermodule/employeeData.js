...
const searchEmpData = async (request, response) => {
    try {
        ...
        let searchData = await db.executeQuery(`select * from users left join roles on users.role_id = roles.id where role_name = ? and (first_name like ? or last_name like ?) and users.status = ?`, ["Employee", search, search,0]);
